...
do
python -m easy_rec_ext.main --pipeline_config_path=xdeepfm_v1.json --task_type=train_and_evaluate \
  --train_input_path=${easy_rec_ext_data_dir}/sample_${part} \
  --eval_input_path=${easy_rec_ext_data_dir}/sample_07
echo "训练模型sample_${part}"
done

python -m easy_rec_ext.tools.check_model_variable --checkpoint_path=xdeepfm_v1/ckpt
