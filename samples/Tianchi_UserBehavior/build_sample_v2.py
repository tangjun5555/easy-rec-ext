...
parser.add_argument("--user_train_num", type=int, required=False, default=20)
...
train_output_files = []
for i in range(args.user_train_num):
    if i < 10:
        train_output_files.append(open(args.output_path + "_train_0%d.txt" % i, mode="w"))
    else:
        train_output_files.append(open(args.output_path + "_train_%d.txt" % i, mode="w"))
    ...
    for line in f:
        ...
        if split[0] != user_id:
            if user_id:
                ...
                for i in range(1, 1 + args.user_train_num):
                    ...
                    if len(user_seq_list) > (i+1):
                        ...
                        history_behavior_list = []
                    train_output_files[args.user_train_num-i].write(
                        ",".join([
                            str(1),
